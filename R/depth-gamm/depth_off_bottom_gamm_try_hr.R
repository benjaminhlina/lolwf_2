...
m <- bam(mean_use ~
           te(hour, doy, k = c(5, 15), bs = c("cc", "cc")) +
           s(tag_serial_number, bs = "re") +
           s(year, bs = "re"),
         family = Gamma(link = "log"),
         method = "fREML",
         data = det_summary %>%
           filter(measurment %in% "converted_sensor"))
draw(m)
appraise(m)
...
fv <- fitted_values(m, data = ds3, scale = "response",
                    exclude = c("s(tag_serial_number", "s(year)"))
...
ggplot() +
  geom_line(data = fv, aes(x = interactionhour, y = .fitted)) +
...
sm <- smooth_estimates(m, select = "te(hour, doy)", data = ds3, partial_match = TRUE) |>
  add_confint()
sm
...
ggsave(filename = here("plots",
                       "depth-from-bottom",
                       "daily_gamm.png"), plot = p,
       width = 11, height = 8.5)
ggplot(data = det_summary %>%
         filter((measurment %in% c("from_bottom"))),
       aes(x = month_abb,
           y = mean_use,
           fill = measurment)) +
